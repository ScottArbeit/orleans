...
public class CosmosGrainStorageHpkIntegrationTests
{
    ...
    public async Task HierarchicalPartitionKeys_SupportSpecialValuesReactivationAndPrefixQueries()
    {
        ...
        try
        {
            ...
            await storage.WriteStateAsync(
                firstGrainType,
                firstGrainId,
                new GrainState<TestState> { State = new TestState { Value = 11 } });
            await storage.WriteStateAsync(
                secondGrainType,
                secondGrainId,
                new GrainState<TestState> { State = new TestState { Value = 22 } });

            var reactivatedStorage = await StartStorage(options, clusterOptions, documentIdProvider);
            var reactivatedState = new GrainState<TestState> { State = new TestState() };
            await reactivatedStorage.ReadStateAsync(firstGrainType, firstGrainId, reactivatedState);
            Assert.True(reactivatedState.RecordExists);
            Assert.Equal(11, reactivatedState.State.Value);

            var container = client.GetContainer(databaseName, containerName);
            var tenantDocuments = await QueryDocuments(
                container,
                new QueryDefinition("SELECT * FROM c WHERE c.PartitionKey = @tenant")
                    .WithParameter("@tenant", tenant),
                BuildPartitionKey([tenant]));
            Assert.Equal(2, tenantDocuments.Count);
            Assert.All(tenantDocuments, document => Assert.Equal(tenant, document.Value<string>("PartitionKey")));

            var currentDocuments = await QueryDocuments(
                container,
                new QueryDefinition(
                    "SELECT * FROM c WHERE c.PartitionKey = @tenant AND c.PartitionKey2 = @grainType")
                    .WithParameter("@tenant", tenant)
                    .WithParameter("@grainType", firstGrainType),
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private static async Task<List<JObject>> QueryDocuments(
        Container container,
    ...
    private sealed class QueryableDocumentIdProvider : IDocumentIdProvider
    {
        private readonly DefaultDocumentIdProvider _defaultProvider;
        private readonly string _tenant;

        public QueryableDocumentIdProvider(IOptions<ClusterOptions> clusterOptions, string tenant)
        {
            _defaultProvider = new DefaultDocumentIdProvider(clusterOptions);
            _tenant = tenant;
        }
        ...
        public ValueTask<(string DocumentId, string PartitionKey)> GetDocumentIdentifiers(string grainType, GrainId grainId) =>
            new((_defaultProvider.GetId(grainType, grainId), _tenant));

        public ValueTask<CosmosDocumentKey> GetDocumentKey(string grainType, GrainId grainId) =>
            new(new CosmosDocumentKey(
                _defaultProvider.GetId(grainType, grainId),
                [_tenant, grainType, grainId.Key.ToString()!]));
    }
